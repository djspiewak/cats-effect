    ...
    "respect self-cancelation" in real {
      dispatcher use { runner =>
        for {
          ...
          _ <- IO(runner.unsafeRunAndForget((IO.canceled >> resultR.set(true)).guarantee(latch.complete(()).void)))
          _ <- latch.get
      ...
      IO.deferred[Unit] flatMap { canceled =>
        IO.deferred[Unit] flatMap { gate =>
          val test = dispatcher use { runner =>
            for {
              _ <- IO(runner.unsafeRunAndForget((gate.complete(()) >> IO.never).onCancel(canceled.complete(()).void)))
              _ <- gate.get
              _ <- IO.raiseError(TestException).void
            } yield ()
          }

          test.handleError(_ => ()) >> canceled.get.as(ok).timeoutTo(2.seconds, IO(false must beTrue))
        }
      }
    }
  }
