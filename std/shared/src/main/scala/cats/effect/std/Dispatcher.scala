      ...
      _ <- {
        def dispatcher(
          ...
          val step = for {
            ...
            _ <-
              if (regs.isEmpty) {
                // this needs to be interruptible even in the await case
                // right now, when await = true, we don't actually whack this because of the inner supervisor
                // note that this is a problem in both parallel and sequential modes
                // we can work around it by using .background rather than the inner supervisor, but
                // then sequential mode self-cancelation (and similar weirdness) becomes fatal
                //
                // we also can't use uncancelable/poll here because it would mask self-cancelation
                // when sequential and await are true, which is not the desired semantic
                F.async_[Unit] { cb =>
