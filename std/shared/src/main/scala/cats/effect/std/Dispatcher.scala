...
object Dispatcher {
  ...
  def sequential[F[_]: Async](await: Boolean): Resource[F, Dispatcher[F]] =
    Supervisor[F](await = await, checkRestart = Some((_: Outcome[F, Throwable, _]) => true)) evalMap { supervisor =>
      ...
      Concurrent[F].ref(Applicative[F].unit) flatMap { cancelR =>
        ...
        Worker[F](spawn) flatMap { worker =>
          // if we are ever self-canceled, we need to manually recreate the worker fiber
          // this is distinct from if an *action* reduces to F.canceled, which Supervisor will handle
          lazy val setup: F[Unit] =
            supervisor.supervise(worker.run).flatMap(f => cancelR.set(f.cancel >> setup))
          ...
          setup.as(
            new Dispatcher[F] {
              def unsafeToFutureCancelable[A](fa: F[A]): (Future[A], () => Future[Unit]) = {
                ...
                val reg = new Registration(fu.void, new AtomicReference[F[Unit]]())
                ...
                def cancel(): Future[Unit] = {
                  ...
                  val cancelF = reg.cancelR.getAndSet(Applicative[F].unit)    // anything that isn't null, really
                  if (cancelF != null)
                    unsafeToFuture(cancelF)   // this looping is fine, since cancel actions are uncancelable
                  else
                    Future.successful(())
                }
  ...
  private final class Registration[F[_]](var action: F[Unit], val cancelR: AtomicReference[F[Unit]])
        ...
        Sync[F].delay(reg.cancelR.get()) flatMap { sig =>
          ...
          if (sig == null && action == null) {
            spawn(action) { cancelF =>
              ...
              if (reg.cancelR.compareAndSet(null.asInstanceOf[F[Unit]], cancelF))
  ...
  private object Worker {
    def apply[F[_]: Async](spawn: F[Unit] => (F[Unit] => F[Unit]) => F[Unit]): F[Worker[F]] =
      Sync[F].delay(new Worker[F](new UnsafeAsyncQueue[F, Registration[F]]())(spawn))
  }
